...
<body>
  ...
  <div>
    <div><a href="${contextPath}/app/user/1">요청1</a></div>
  ...
  <c:if test="${empty sessionScope.loginUser}">
    <div>
      <form action="${contextPath}/user/login" method="post">
        ...
        <div>
          <button type="submit">LOG IN</button> <%-- 버튼이 form 태그 밖에 있었음. 신경쓰기 --%>
        </div>
      </form>
    </div>
  </c:if>
  ...
  <c:if test="${not empty sessionScope.memberVO}">
    <div><a href="${contextPath}/member/mypage">${sessionScope.memberVO.id} 님 어서오세요.</a></div>
    <div><a href="${contextPath}/member/logout">LOG OUT</a></div> <%-- 수정 --%>
  </c:if>
